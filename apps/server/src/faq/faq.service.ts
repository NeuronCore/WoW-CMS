...
export class FaqService
{
    ...
    public async create(createFaqDto: CreateFaqDto)
    {
        ...
        await this.webDatabase.execute('INSERT INTO `faq` (`title`, `description`) VALUES (?, ?)', [title, description]);
    ...
    public async update(id: number, updateFaqDto: UpdateFaqDto)
    {
        try
        {
            ...
            const [faq] = await this.webDatabase.query('SELECT `title`, `description` FROM `faq` WHERE `id` = ?', [id]);
            if (!faq[0])
                return { statusCode: HttpStatus.NOT_FOUND, message: 'FAQ with this id not found' };

            await this.webDatabase.execute('UPDATE `faq` SET `title` = ?, `description` = ? WHERE `id` = ?', [title || faq[0].title, description || faq[0].description, id]);
